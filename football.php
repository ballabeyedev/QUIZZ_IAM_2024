...
try {
    ...
    die("Erreur de la basse de donnee: " . $e->getMessage());
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['answers']) && is_array($_POST['answers'])) {
        ...
        if (isset($_SESSION['user_email'])) {
            ...
            $stmtUpdateScore = $connect->prepare("UPDATE joueur SET score_joueur = :score WHERE email = :userEmail");
            $stmtUpdateScore->bindParam(':score', $score, PDO::PARAM_INT);
            $stmtUpdateScore->bindParam(':userEmail', $userEmail, PDO::PARAM_STR);

            if ($stmtUpdateScore->execute()) {
                echo "Score updated successfully!";
            } else {
                echo "Error updating score: " . implode(", ", $stmtUpdateScore->errorInfo());
            }
        }
    }
}
...
$stmtQuestions->execute(['theme' => 'football']);
...
<head>
    ...
    <style>
        ...
        form { max-width: 700px; margin-top: -500px; background-color: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); margin-left: 100px; width: 600px}
        ...
        .soum { background-color:rgb(139, 0, 139); color: #fff; padding: 10px 15px; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; }        button:hover { background-color: #45a049; }
        ...
        .logo { display: flex; align-items: center; margin-bottom: 30px; background-color: #3498db; padding: 10px; width: 180px; height: 550px; }
        ...
        span {
            color: #3498db; 
            font-weight: bold;
            animation: highlight 2s ease-in-out infinite alternate; /* Animation CSS */
        }
...
<body>
    ...
    <div class="text">
        <h3>VOUS AVEEZ CHOISIE L'OPTION: <span>FOOTBALL</span></h3>
    </div>
    <div class="div">
    <form action='football.php' method='post'>
        ...
        <div>
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                echo "<p><a href='reponse1.php'>Je veux voir mes reponses correcte et incorrecte !</a>";
            }
            ?>
        </div>
    </form>
